...
`thread-algo-search-lab` is centered on algorithms work in OCaml. The technical target is: Package an OCaml local lab for search analysis with capacity fixtures, allocation and spill reports, and documented operating limits.
...
The score is deterministic. The current threshold is 155. Risk has a penalty of 5, latency has a penalty of 2, and weight contributes a bonus of 3.
